...
export default function Weight() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(t('weight.addSuccess'));
      setShowForm(false);
      setAverageWeight('');
      fetchInitialData();
    } catch (error) {
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center bg-white p-3 rounded-xl shadow-sm border border-gray-100">
        <h2 className="text-xl font-bold text-gray-800 flex items-center gap-2">
          <Scale className="text-yellow-600" /> {t('weight.title')}
        </h2>
        <button 
          onClick={() => {
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-white p-4 rounded-xl shadow border border-yellow-100 space-y-3">
          <div>
            ...
            <select required value={batchId} onChange={(e) => handleBatchChange(e.target.value)} className="w-full border border-gray-300 rounded-lg p-2 focus:ring-2 focus:ring-yellow-500 bg-white">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">{t('weight.ageDays')}</label>
              <input required type="number" value={ageDays} onChange={(e) => setAgeDays(e.target.value)} className="w-full border border-gray-300 rounded-lg p-2 focus:ring-2 focus:ring-yellow-500" placeholder={t('weight.ageDays')} />
            </div>
          </div>
          
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">{t('weight.avgWeight')}</label>
              <input required type="number" value={averageWeight} step="any" onChange={(e) => setAverageWeight(e.target.value)} className="w-full border border-gray-300 rounded-lg p-2 focus:ring-2 focus:ring-yellow-500" placeholder="e.g. 1200" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">{t('weight.birdsSampled')}</label>
      ...
      <div className="space-y-3">
        {records.length === 0 ? (
           <div className="text-center text-gray-500 py-6 bg-white rounded-xl border border-dashed border-gray-300">
             {t('dues.dueEmpty').replace('due', 'weight')}
           </div>
        ) : records.map(record => {
          ...
          return (
            <div key={record.id} className="bg-white p-3 rounded-xl border border-gray-100 shadow-sm flex items-start justify-between">
              <div>
                ...
                <p className="text-xs text-gray-500">{new Date(record.date).toLocaleDateString()} • {t('weight.ageDays')}: <span className="font-semibold text-gray-700">{record.ageDays} {t('weight.ageDays').includes('দিন') ? 'দিন' : 'days'}</span></p>
                <div className="mt-1 text-sm bg-yellow-50 text-yellow-800 px-2 py-1 rounded inline-block">
                  {t('weight.birdsSampled').split('?')[0]}: {record.totalBirdsSampled} {t('mortality.qtyLabel')}
                </div>
              </div>
              <div className="text-right flex flex-col items-end">
                <span className="font-bold text-yellow-600 text-lg">{record.averageWeight} {t('weight.avgWeight').includes('গ্রাম') ? 'গ্রাম' : 'grams'}</span>
                <span className="text-[10px] text-gray-400 mt-1">{t('weight.avgWeight')}</span>
